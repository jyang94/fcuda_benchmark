...
module AESL_autobus_C #(
	parameter DATA_WIDTH = 32'd32,
	parameter ADDR_WIDTH = 32'd32,
	parameter DEPTH = 32'd10240,
	parameter FIFO_rsp_DEPTH = 32'd10240,
	...
	localparam TV_IN  = "../tv/cdatafile/c.matrixMul.autotvout_C.dat";
	localparam TV_OUT = "../tv/rtldatafile/rtl.matrixMul.autotvout_C.dat";
	...
	reg [DATA_WIDTH - 1:0] mem [0:DEPTH * mem_page_num - 1];
	
	// initial begin : initialize_mem
	// 	integer i;
	// 	for (i = 0; i < DEPTH * mem_page_num; i = i + 1) begin
	// 		mem[i] = 0;
	// 	end
	// end
		...
		forever begin
			...
			while (pending_write && ~fifo_req.empty) begin
				...
				if (pending_address < DEPTH) begin
					mem[pending_address + (pending_transaction % mem_page_num) * DEPTH] = req_din;
				end else begin
			...
			while (pending_read && ~fifo_rsp.full) begin
				if (pending_address < DEPTH) begin
					rsp_dout = mem[pending_address + (pending_transaction % mem_page_num) * DEPTH];
				end else begin
		...
		if (done_1 == 1) begin
			if (transaction_save < 1) begin
				...
				for (i = 0; i < DEPTH; i = i + 1) begin
					$fdisplay(fp, "0x%x", mem[i + base_page]);
				end
	...
	forever begin
		...
		if (ready == 1) begin
			if (token != "[[[/runtime]]]") begin
				...
				for (i = 0; i < DEPTH; i = i + 1) begin
					token = read_token(fp);
					ret = $sscanf(rm_0x(token), "%x", mem[i + base_page]);
				end
				token = read_token(fp);
				if (token != "[[/transaction]]") begin
					$display("ERROR: Simulation using HLS TB failed.");
